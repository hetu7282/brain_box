...
import 'package:brain_box/src/features/kings_gambit/presentation/logic/move_calculation/move_classes/move_stack_object.dart';
import 'package:brain_box/src/features/kings_gambit/presentation/logic/move_calculation/openings.dart';
...
MoveMeta push(
  ...
  if (_castled(mso.movedPiece, mso.takenPiece)) {
    ...
    // Track captured piece info in meta
    if (meta.took && mso.takenPiece != null) {
      meta.capturedPieceType = mso.takenPiece!.type;
      meta.capturedPiecePlayer = mso.takenPiece!.player;
    }
    if (mso.movedPiece?.type == ChessPieceType.pawn) {
      ...
      if (mso.enPassant && mso.takenPiece != null) {
        meta.capturedPieceType = mso.takenPiece!.type;
        meta.capturedPiecePlayer = mso.takenPiece!.player;
      }
    }
    ...
    case ChessPieceType.queen:
      {
        if (mso.movedPiece != null) {
          _queensForPlayer(
            mso.movedPiece?.player ?? Player.player1,
            board,
          ).add(mso.movedPiece!);
        }
      }
...
void _checkMoveAmbiguity(Move move, MoveMeta moveMeta, ChessBoard board) {
  ...
  for (var otherPiece in _piecesOfTypeForPlayer(
    piece?.type,
    piece?.player,
    board,
  )) {
